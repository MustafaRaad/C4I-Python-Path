...
class Application(Frame):
    def __init__(self, master):
        ...
        self.entry = Entry(master, width=26, font=("Serif",25))
        self.entry.grid(row=0, column=0, columnspan=5, sticky="w")
        self.entry.focus_set()
        #self.ofy = PhotoImage(file='ofy.jpg')
        self.entry.configure(state="disabled", disabledbackground="light blue", disabledforeground="black")
        self.create_widgets()
        self.bind_buttons(master)
        # self.entry.config(image=ofy,compound=CENTER)
        self.grid()
    ...
    def create_widgets(self):
        ...
        self.ac_bttn = Button(self, text='Clean', width=9, height=3, command=lambda: self.clear_all())
        self.ac_bttn.grid(row=4, column=0)

        self.c_bttn = Button(self, text='←', width=9, height=3, command=lambda: self.clear())
        self.c_bttn.grid(row=1, column=4 )
